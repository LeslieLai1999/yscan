...
def icpinfo(pid,icpinfo_page):
    ...
    for i in range(1,icpinfo_page+1):
        try:
            ...
            res = requests.get(url=invest_url, headers=headers2, verify=False,
                               timeout=TIMEOUT)
...
def invest(pid,invest_page):
    ...
    for i in range(1,invest_page+1):
        try:
            ...
            res = requests.get(url=invest_url, headers=headers2, verify=False, timeout=TIMEOUT)
            text = res.text
            text = json.loads(text)
            data = text["data"]
            for each in data["list"]:
...
def holds(pid,holds_page):
    ...
    for i in range(1, holds_page + 1):
        try:
            ...
            res = requests.get(url=holds_url, headers=headers2, verify=False, timeout=TIMEOUT)
            text = res.text
            text = json.loads(text)
            data = text["data"]
            # print(data)
...
def branch(pid,branch_page):
    ...
    for i in range(1, branch_page + 1):
        try:
            ...
            res = requests.get(url=branch_url, headers=headers2, verify=False,
                               timeout=TIMEOUT)
            text = res.text
            text = json.loads(text)
            data = text["data"]
...
def start(searchContent):
    url =     url = 'https://aiqicha.baidu.com/index/suggest'
    ...
    res = requests.get(url=url, headers=headers2, verify=False, timeout=TIMEOUT)
    text = res.text
...
def run_Aiqicha(conpanynName):
    selfIcpinfo_infos, invest_infos, holds_infos, branch_infos = start(conpanynName)
