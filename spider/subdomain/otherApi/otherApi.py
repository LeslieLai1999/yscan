...
def init(_,requests_proxies):
    global virustotalApi, securitytrailsApi, headers, domain,TIMEOUT,proxies
...
def fht_api():
    ...
    try:
        print(proxies)
        res = requests.get(url=url, headers=headers, verify=False, timeout=30,proxies = proxies)
...
def sublist3r_api():
    ...
    try:
        res = requests.get(url=url, headers=headers, verify=False, timeout=TIMEOUT,proxies=proxies)
        text = res.text
        if text != 'null':
...
def bufferover_api():
    ...
    while error_num < 10 and not reqError:
        try:
            ...
            res = scraper.get(url=url, headers=headers, verify=False, timeout=TIMEOUT,proxies=proxies)
            text = res.text
...
def threatcrowd_api():
    ...
    while error_num < 10 and not reqError:      # 当没有绕过Cloudflare验证码时，并且重复次数小于50次时，一直循环
        try:
            ...
            res = scraper.get(url=url, headers=headers, timeout=TIMEOUT,proxies=proxies)
            text = res.text
...
def ximcx_api():
    ...
    ximcx_subdomains = []
    try:
        res = requests.post(url=url, data=data, headers=headers, verify=False, timeout=TIMEOUT,proxies=proxies)
        text = res.text
...
def otherApi(domain,requests_proxies):
    init(domain,requests_proxies)
...
if __name__ == '__main__':
    ...
    othersApiTotalSubdomains =  otherApi('zjhu.edu.cn',proxy)
