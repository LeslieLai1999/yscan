...
cdnDict = {}
with open(getRootPath()+'spider/subdomain/CDN/cdn-domain.conf', 'rt', encoding='utf-8') as f:
...
def ipASNSCheckCDN(subdomain):
    ...
    with geoip2.database.Reader(getRootPath()+'spider/subdomain/CDN/GeoLite2-ASN.mmdb') as reader:
...
def check(subdomains_queue, notCDNSubdomains, CDNSubdomainsDict):
...
def checkCDN(subdomains):
    ...
    for t_id in range(50):  # 对新增的子域名进行A记录查询获取IP
        t = Thread(target=check, args=(subdomains_queue, notCDNSubdomains, CDNSubdomainsDict))
